...
public final class VictoryApi { private static final List<Entry> RULES=new ArrayList<>();private static long order;private VictoryApi(){}
 ...
 public interface VictoryRule{VictoryResult evaluate(Context c);} public record Context(ServerWorld world,GameWorldComponent gameWorld,List<ServerPlayerEntity> alivePlayers,GameFunctions.WinStatus vanillaWinStatus){}
 public enum Action{PASS,KEEP_RUNNING,VANILLA_WIN,CUSTOM_WIN} public record VictoryResult(Action action,GameFunctions.WinStatus winStatus,CustomVictory customVictory,List<UUID> extraWinnerUuids){public static VictoryResult pass(){return new VictoryResult(Action.PASS,null,null,List.of());}public static VictoryResult keepRunning(){return new VictoryResult(Action.KEEP_RUNNING,null,null,List.of());}public static VictoryResult vanillaWin(GameFunctions.WinStatus s,Collection<UUID>e){return new VictoryResult(Action.VANILLA_WIN,s,null,List.copyOf(e));}public static VictoryResult customWin(CustomVictory v){return new VictoryResult(Action.CUSTOM_WIN,null,v,List.of());}}
 private record Entry(Identifier id,int priority,long order,VictoryRule rule){}
}
